    ...
    public:
            ...
            const label_t tid) noexcept
        {
            device::constexpr_for<0, (VelocitySet::Q() - 1)>(
                [&](const auto q_)
                {
                    s_pop[label_constant<q_ * block::stride()>() + tid] = pop(q_i<q_ + 1>());
                });
        }
            ...
            const thread::array<scalar_t, N> &s_pop) noexcept
        {
            ...
            pop(q_i<1>()) = s_pop[label_constant<0 * block::stride()>() + device::idxBlock(xm1, threadIdx.y, threadIdx.z)];
            pop(q_i<2>()) = s_pop[label_constant<1 * block::stride()>() + device::idxBlock(xp1, threadIdx.y, threadIdx.z)];
            pop(q_i<3>()) = s_pop[label_constant<2 * block::stride()>() + device::idxBlock(threadIdx.x, ym1, threadIdx.z)];
            pop(q_i<4>()) = s_pop[label_constant<3 * block::stride()>() + device::idxBlock(threadIdx.x, yp1, threadIdx.z)];
            pop(q_i<5>()) = s_pop[label_constant<4 * block::stride()>() + device::idxBlock(threadIdx.x, threadIdx.y, zm1)];
            pop(q_i<6>()) = s_pop[label_constant<5 * block::stride()>() + device::idxBlock(threadIdx.x, threadIdx.y, zp1)];
            pop(q_i<7>()) = s_pop[label_constant<6 * block::stride()>() + device::idxBlock(xm1, ym1, threadIdx.z)];
            pop(q_i<8>()) = s_pop[label_constant<7 * block::stride()>() + device::idxBlock(xp1, yp1, threadIdx.z)];
            pop(q_i<9>()) = s_pop[label_constant<8 * block::stride()>() + device::idxBlock(xm1, threadIdx.y, zm1)];
            pop(q_i<10>()) = s_pop[label_constant<9 * block::stride()>() + device::idxBlock(xp1, threadIdx.y, zp1)];
            pop(q_i<11>()) = s_pop[label_constant<10 * block::stride()>() + device::idxBlock(threadIdx.x, ym1, zm1)];
            pop(q_i<12>()) = s_pop[label_constant<11 * block::stride()>() + device::idxBlock(threadIdx.x, yp1, zp1)];
            pop(q_i<13>()) = s_pop[label_constant<12 * block::stride()>() + device::idxBlock(xm1, yp1, threadIdx.z)];
            pop(q_i<14>()) = s_pop[label_constant<13 * block::stride()>() + device::idxBlock(xp1, ym1, threadIdx.z)];
            pop(q_i<15>()) = s_pop[label_constant<14 * block::stride()>() + device::idxBlock(xm1, threadIdx.y, zp1)];
            pop(q_i<16>()) = s_pop[label_constant<15 * block::stride()>() + device::idxBlock(xp1, threadIdx.y, zm1)];
            pop(q_i<17>()) = s_pop[label_constant<16 * block::stride()>() + device::idxBlock(threadIdx.x, ym1, zp1)];
            pop(q_i<18>()) = s_pop[label_constant<17 * block::stride()>() + device::idxBlock(threadIdx.x, yp1, zm1)];

            if constexpr (VelocitySet::Q() == 27)
            {
                pop(q_i<19>()) = s_pop[label_constant<18 * block::stride()>() + device::idxBlock(xm1, ym1, zm1)];
                pop(q_i<20>()) = s_pop[label_constant<19 * block::stride()>() + device::idxBlock(xp1, yp1, zp1)];
                pop(q_i<21>()) = s_pop[label_constant<20 * block::stride()>() + device::idxBlock(xm1, ym1, zp1)];
                pop(q_i<22>()) = s_pop[label_constant<21 * block::stride()>() + device::idxBlock(xp1, yp1, zm1)];
                pop(q_i<23>()) = s_pop[label_constant<22 * block::stride()>() + device::idxBlock(xm1, yp1, zm1)];
                pop(q_i<24>()) = s_pop[label_constant<23 * block::stride()>() + device::idxBlock(xp1, ym1, zp1)];
                pop(q_i<25>()) = s_pop[label_constant<24 * block::stride()>() + device::idxBlock(xp1, ym1, zm1)];
                pop(q_i<26>()) = s_pop[label_constant<25 * block::stride()>() + device::idxBlock(xm1, yp1, zp1)];
            }
        }
