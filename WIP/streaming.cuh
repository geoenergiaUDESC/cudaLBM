...
namespace LBM
{
    ...
    public:
        ...
        __device__ static inline void phase_pull(
            thread::array<scalar_t, PhaseVelocitySet::Q()> &phase_pop,
            const thread::array<scalar_t, N> &s_phase_pop) noexcept
        {
            ...
            phase_pop(label_constant<1>()) = s_phase_pop[label_constant<0 * block::stride()>() + device::idxBlock(xm1, threadIdx.y, threadIdx.z)];
            phase_pop(label_constant<2>()) = s_phase_pop[label_constant<1 * block::stride()>() + device::idxBlock(xp1, threadIdx.y, threadIdx.z)];
            phase_pop(label_constant<3>()) = s_phase_pop[label_constant<2 * block::stride()>() + device::idxBlock(threadIdx.x, ym1, threadIdx.z)];
