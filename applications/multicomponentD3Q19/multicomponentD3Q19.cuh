...
#ifndef __MBLBM_MULTICOMPONENTD3Q19_CUH
#define __MBLBM_MULTICOMPONENTD3Q19_CUH
...
#define launchBoundsD3Q19 __launch_bounds__(block::maxThreads(), MIN_BLOCKS_PER_MP())
     ...
     * @param hydroPtrs Collection of 10 pointers to device arrays on the GPU
     * @param phasePtrs Collection of 1 pointer to device arrays on the GPU
     * @param blockHalo Object containing pointers to the block halo faces used to exchange the population densities
     **/
    launchBoundsD3Q19 __global__ void multicomponentD3Q19(
        const device::ptrCollection<10, scalar_t> hydroPtrs,
        const device::ptrCollection<1, scalar_t> phasePtrs,
        const device::ptrCollection<6, const scalar_t> f_fGhost,
        const device::ptrCollection<6, scalar_t> f_gGhost,
        const device::ptrCollection<6, const scalar_t> g_fGhost,
        const device::ptrCollection<6, scalar_t> g_gGhost)
    {
        ...
        extern __shared__ scalar_t f_shared_buffer[];
        extern __shared__ scalar_t g_shared_buffer[];
        ...
        thread::array<scalar_t, VelocitySet::Q()> f_pop = VelocitySet::reconstruct(moments);
        thread::array<scalar_t, VelocitySet::Q()> g_pop = VelocitySet::reconstruct(moments);
