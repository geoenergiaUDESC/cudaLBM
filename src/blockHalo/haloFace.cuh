...
namespace LBM
{
    namespace device
    {
        ...
        public:
             ...
             * @param fMom An std::vector of std::vectors of type scalar_t containing the 10 interlaced moments
             * @param mesh The mesh
             **/
        ...
        private:
            ...
            __host__ [[nodiscard]] static inline constexpr label_t nFaces(const host::latticeMesh &mesh) noexcept
            {
                ...
                {
                    return ((mesh.nx() * mesh.ny() * mesh.nz()) / block::nx()) * VSet::QF();
                }
                if constexpr (faceIndex == device::haloFaces::y())
                {
                    return ((mesh.nx() * mesh.ny() * mesh.nz()) / block::ny()) * VSet::QF();
                }
                if constexpr (faceIndex == device::haloFaces::z())
                {
                    return ((mesh.nx() * mesh.ny() * mesh.nz()) / block::nz()) * VSet::QF();
                }
            ...
            /**
             * @brief Initialises the halo points for a particular mesh point
             * @param face An std::vector of scalar_t of halo points at a particular block face
             * @param pop The population density
             * @param tx, ty, tz Thread indices
            ...
            __host__ void static handleGhostCells(
                ...
                const std::array<scalar_t, VSet::Q()> &pop,
                ...
                const host::latticeMesh &mesh) noexcept
            {
                if constexpr (faceIndex == device::haloFaces::x())
                {
                    if constexpr (side == 0)
                    {
                        if (tx == 0)
                        { // w
                            face[host::idxPopX<0, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[2];
                            face[host::idxPopX<1, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[8];
                            face[host::idxPopX<2, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[10];
                            face[host::idxPopX<3, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[14];
                            face[host::idxPopX<4, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[16];
                        }
                    }
                    if constexpr (side == 1)
                    {
                        if (tx == (block::nx() - 1))
                        {
                            face[host::idxPopX<0, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[1];
                            face[host::idxPopX<1, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[7];
                            face[host::idxPopX<2, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[9];
                            face[host::idxPopX<3, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[13];
                            face[host::idxPopX<4, VSet::QF()>(ty, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[15];
                        }
                    }
                }
                ...
                if constexpr (faceIndex == device::haloFaces::y())
                {
                    if constexpr (side == 0)
                    {
                        if (ty == 0)
                        { // s
                            face[host::idxPopY<0, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[4];
                            face[host::idxPopY<1, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[8];
                            face[host::idxPopY<2, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[12];
                            face[host::idxPopY<3, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[13];
                            face[host::idxPopY<4, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[18];
                        }
                    }
                    if constexpr (side == 1)
                    {
                        if (ty == (block::ny() - 1))
                        {
                            face[host::idxPopY<0, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[3];
                            face[host::idxPopY<1, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[7];
                            face[host::idxPopY<2, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[11];
                            face[host::idxPopY<3, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[14];
                            face[host::idxPopY<4, VSet::QF()>(tx, tz, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[17];
                        }
                    }
                }
                ...
                if constexpr (faceIndex == device::haloFaces::z())
                {
                    if constexpr (side == 0)
                    {
                        if (tz == 0)
                        { // b
                            face[host::idxPopZ<0, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[6];
                            face[host::idxPopZ<1, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[10];
                            face[host::idxPopZ<2, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[12];
                            face[host::idxPopZ<3, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[15];
                            face[host::idxPopZ<4, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[17];
                        }
                    }
                    if constexpr (side == 1)
                    {
                        if (tz == (block::nz() - 1))
                        {
                            face[host::idxPopZ<0, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[5];
                            face[host::idxPopZ<1, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[9];
                            face[host::idxPopZ<2, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[11];
                            face[host::idxPopZ<3, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[16];
                            face[host::idxPopZ<4, VSet::QF()>(tx, ty, bx, by, bz, mesh.nxBlocks(), mesh.nyBlocks())] = pop[18];
                        }
                    }
                }
            }
