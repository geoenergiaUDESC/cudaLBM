...
namespace LBM
{
    ...
    public:
        ...
        __device__ static inline void collide(scalar_t (&ptrRestrict moments)[10]) noexcept
        {
            ...
            moments[4] = device_t_omegaVar * moments[4] + device_omegaVar_d2 * (moments[1]) * (moments[1]);
            moments[7] = device_t_omegaVar * moments[7] + device_omegaVar_d2 * (moments[2]) * (moments[2]);
            moments[9] = device_t_omegaVar * moments[9] + device_omegaVar_d2 * (moments[3]) * (moments[3]);

            // Off-diagonal moment updates (remove force terms)
            moments[5] = device_t_omegaVar * moments[5] + device_omega * (moments[1]) * (moments[2]);
            moments[6] = device_t_omegaVar * moments[6] + device_omega * (moments[1]) * (moments[3]);
            moments[8] = device_t_omegaVar * moments[8] + device_omega * (moments[2]) * (moments[3]);
        }
