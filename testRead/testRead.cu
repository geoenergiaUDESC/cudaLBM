...
// const std::string NAME = "rho";

void write(const std::vector<std::string> &S) noexcept
{
    for (label_t line = 0; line < S.size(); line++)
    {
...
public:
    ...
    [[nodiscard]] boundaryValue(const std::string &fieldName, const std::string &regionName)
        : value(initialiseValue(fieldName, regionName)) {};

    [[nodiscard]] inline constexpr scalar_t operator()() const noexcept
    {
        return value;
    }
...
private:
    ...
    [[nodiscard]] scalar_t initialiseValue(const std::string &fieldName, const std::string &regionName, const std::string &initialConditionsName = "initialConditions") const
    {
        ...
        const std::vector<std::string> fieldBlock = extractBlock(boundaryLines, fieldName, "field");

        // Extracts the block of text corresponding to internalField within the current field block
        const std::vector<std::string> internalFieldBlock = extractBlock(fieldBlock, regionName);
        ...
        else if (value_ == "equilibrium")
        {
            ...
            if (isMember)
            {
                ...
                const std::array<scalar_t, VelocitySet::D3Q19::Q()> pop = VelocitySet::D3Q19::F_eq(u.value, v.value, w.value);
            ...
            {
                std::cout << "Entry for " << fieldName << " in region " << regionName << " not a valid numerical value and not an equilibrium moment" << std::endl;
...
private:
    const boundaryValue values_[10];
};
...
public:
    [[nodiscard]] boundaryFields(const std::string &fieldName)
        : values_{
              boundaryValue(fieldName, "North"),
              boundaryValue(fieldName, "South"),
              boundaryValue(fieldName, "East"),
              boundaryValue(fieldName, "West"),
              boundaryValue(fieldName, "Front"),
              boundaryValue(fieldName, "Back"),
              boundaryValue(fieldName, "internalField")},
...
{
    const boundaryFields field_b(fieldName);
...
int main(const int argc, const char *const argv[])
{
    ...
    const std::vector<scalar_t> rho = initialise_from_boundary_conditions(mesh, "rho");
    const std::vector<scalar_t> u = initialise_from_boundary_conditions(mesh, "u");
    const std::vector<scalar_t> v = initialise_from_boundary_conditions(mesh, "v");
    const std::vector<scalar_t> w = initialise_from_boundary_conditions(mesh, "w");

    const std::vector<scalar_t> m_xx = initialise_from_boundary_conditions(mesh, "m_xx");
    const std::vector<scalar_t> m_xy = initialise_from_boundary_conditions(mesh, "m_xy");
    const std::vector<scalar_t> m_xz = initialise_from_boundary_conditions(mesh, "m_xz");
    const std::vector<scalar_t> m_yy = initialise_from_boundary_conditions(mesh, "m_yy");
    const std::vector<scalar_t> m_yz = initialise_from_boundary_conditions(mesh, "m_yz");
    const std::vector<scalar_t> m_zz = initialise_from_boundary_conditions(mesh, "m_zz");
